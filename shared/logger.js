...
class Logger {
    ...
    exportDirectory = path.join(logDirectory, 'exports');

    // Rate Limiting Configuration
    RATE_LIMIT_INTERVAL = 60000; // 60 seconds
    DISCORD_MESSAGES_SENT = new Map();

    // Setup Client Variable
    Client = null
    ...
    logEvent(eventName, eventCategory = 'General', eventData = '') {
    ...
    appendLogToFile(logFileName, logEntry) {
        ensureLogFile(logFileName);
        const logFilePath = path.join(logDirectory, `${logFileName}.log`);
    ...
    sendLogToDiscord(logEntry, channelId = config.discord.discordConsoleId) {
        ...
        const logChannel = self.Client.channels.cache.get(channelId);
        if (logChannel) {
